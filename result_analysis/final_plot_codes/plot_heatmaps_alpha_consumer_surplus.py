...
for run_folder in selected_folders:
    ...
    # Extract consumer surplus matrix
    ...
    for i, strat_A in enumerate(strategies):
        for j, strat_B in enumerate(strategies):
            if strat_A in welfare_data["welfare_matrix"] and strat_B in welfare_data["welfare_matrix"][strat_A]:
                # Use consumer_surplus instead of producer_profit
                matrix[i, j] = welfare_data["welfare_matrix"][strat_A][strat_B]["consumer_surplus"]
...
output_path_png = os.path.join(base_dir, 'consumer_surplus_heatmaps_alpha_individual.png')
output_path_pdf = os.path.join(base_dir, 'consumer_surplus_heatmaps_alpha_individual.pdf')
...
print("\nConsumer Surplus ranges for each alpha:")
print("-" * 60)
for data in heatmap_data:
    vmin = data['matrix'].min()
    vmax = data['matrix'].max()
    print(f"α={data['alpha']:.2f}: min={vmin:.2f}, max={vmax:.2f}, range={vmax-vmin:.2f}")

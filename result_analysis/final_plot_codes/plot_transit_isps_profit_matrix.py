...
# Read welfare matrix for profit data
...
# Plot heatmap with color based on total profit
vmin = profit_matrix.min()
vmax = profit_matrix.max()
im = ax.imshow(profit_matrix, cmap='plasma', aspect='equal', vmin=vmin, vmax=vmax)
...
for i in range(10):
    for j in range(10):
        ...
        if profit_matrix[i, j] < (vmin + vmax) / 2:
            text_color = 'white'
        else:
            text_color = 'black'
...
cbar.set_label('Transit ISPs Profit', fontsize=12, fontweight='bold')
cbar.ax.tick_params(labelsize=10)

plt.tight_layout()

# Save output
output_path_png = os.path.join(run_path, 'transit_isps_profit_matrix.png')
output_path_pdf = os.path.join(run_path, 'transit_isps_profit_matrix.pdf')
plt.savefig(output_path_png, dpi=300, bbox_inches='tight')
plt.savefig(output_path_pdf, bbox_inches='tight')

print(f"Transit ISPs Profit Matrix saved to:")
print(f"  PNG: {output_path_png}")
print(f"  PDF: {output_path_pdf}")
print(f"\nProfit range: min={vmin:.2f}, max={vmax:.2f}")

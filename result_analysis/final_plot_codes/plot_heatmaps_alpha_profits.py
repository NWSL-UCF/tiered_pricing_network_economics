...
for run_folder in selected_folders:
    ...
    # Extract total profit matrix (sum of both firms' profits)
    ...
    for i, strat_A in enumerate(strategies):
        for j, strat_B in enumerate(strategies):
            if strat_A in welfare_data["welfare_matrix"] and strat_B in welfare_data["welfare_matrix"][strat_A]:
                # Use producer_profit (total profit) instead of social_welfare
...
# Save the plot
output_path = os.path.join(base_dir, 'producer_profit_heatmaps_alpha_individual.png')

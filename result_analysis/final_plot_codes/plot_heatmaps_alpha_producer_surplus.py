    ...
    for i, strat_A in enumerate(strategies):
        for j, strat_B in enumerate(strategies):
            if strat_A in welfare_data["welfare_matrix"] and strat_B in welfare_data["welfare_matrix"][strat_A]:
                # Use producer_profit (producer surplus)
...
output_path_png = os.path.join(base_dir, 'producer_surplus_heatmaps_alpha_individual.png')
output_path_pdf = os.path.join(base_dir, 'producer_surplus_heatmaps_alpha_individual.pdf')
...
print("\nProducer Surplus ranges for each alpha:")
print("-" * 60)

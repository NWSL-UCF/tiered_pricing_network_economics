...
for i, alpha in enumerate(sorted(all_data.keys())):
    ...
    # Plot line
    plt.plot(tiers, sw_normalized, marker='o', label=f'α = {alpha:.2f}', 
             alpha=0.8, linewidth=2.5, markersize=8, color=colors[i])
    ...
    for nash_tier in nash_indices:
        if nash_tier in tiers:
            ...
            plt.scatter(nash_tier, sw_normalized[idx], marker='*', s=600, 
...
plt.legend(fontsize=13, frameon=True, shadow=True, loc='best')
plt.xticks(range(1, 11), ['1x1', '2x2', '3x3', '4x4', '5x5', '6x6', '7x7', '8x8', '9x9', '10x10'], fontsize=12)
plt.yticks(fontsize=12)
plt.tight_layout()

# Save the plot
output_path = os.path.join(base_dir, 'social_welfare_vs_tiers_selected.png')

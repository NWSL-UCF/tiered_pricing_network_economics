...
# Create figure with subplots - add space for individual colorbars
...
for idx, (ax, data) in enumerate(zip(axes, heatmap_data)):
    # Each heatmap uses its own min/max for color scaling
    vmin = data['matrix'].min()
    vmax = data['matrix'].max()
    
    # Plot heatmap
    im = ax.imshow(data['matrix'], cmap='plasma', aspect='auto', vmin=vmin, vmax=vmax)
    ...
    ax.set_title(f'α={data["alpha"]:.2f}', fontsize=10, pad=5)
    ...
    # Add individual colorbar below each heatmap
...
output_path_png = os.path.join(base_dir, 'social_welfare_heatmaps_alpha_individual.png')
output_path_pdf = os.path.join(base_dir, 'social_welfare_heatmaps_alpha_individual.pdf')
...
# Print value ranges for each alpha
print("\nValue ranges for each alpha:")
print("-" * 60)
for data in heatmap_data:
    vmin = data['matrix'].min()
    vmax = data['matrix'].max()
    print(f"α={data['alpha']:.2f}: min={vmin:.6f}, max={vmax:.6f}, range={vmax-vmin:.6f}")

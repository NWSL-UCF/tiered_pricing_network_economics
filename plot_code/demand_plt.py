...
df['tier'] = pd.cut(df['distance'], bins=[0, 500, 2000, float('inf')], labels=['Metro', 'Regional', 'Intercontinental'])
...
# Define price range
price_range = np.linspace(1, 80, 100)
...
plt.tight_layout()
plt.savefig("../figure/demand_curve_by_tier_new.png", dpi=300)
plt.close()
...
plt.grid(True, linestyle='--', alpha=0.6)
plt.tight_layout()
plt.savefig("../figure/demand_curve_zoomed_up_to_20_new.png", dpi=300)
plt.close()
